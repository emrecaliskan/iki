...
class Ftx:
    ...
    @classmethod
    def get_hist_prices(cls, symbol: str, resolution: int, start: dt.datetime, end: dt.datetime):
        exch_symbol = cls.symbol_map[symbol]
        params = {
            'resolution': resolution,
            'start_time': int(start.timestamp()),
            'end_time': int(end.timestamp())
        }
        r = requests.get(f"{cls.URL}/markets/{exch_symbol}/candles", params=params)
        r.raise_for_status()  # harsh but simple
        return r.json()
...
class FtxWebsocketClient(WebsocketManager):
    ...
    def __init__(self, tick_callback, trades_callback) -> None:
        super().__init__()
        self._handle_ticker_message = tick_callback
        self._handle_trades_message = trades_callback
